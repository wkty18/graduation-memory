...
**使用说明（谁可以在线改什么）**：
- **所有人**（未登录）：浏览全站、打开密码信（密码正确时）、看留言
- **班级成员**（注册并登录后）：在「留言」页留言；在**同学个人页的「TA 的留言」**给别人留言——保存即写入云端，所有人立刻可见
- **管理员**（admins 表内账号，仅你）：编辑同学名单（含**上传同学头像**）、信件管理（编辑/设密码/删除）、快捷操作
- 同学名单与信件的修改权**仅管理员**（数据库层 RLS 强制，成员直连也会被拒）
